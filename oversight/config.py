...
class SafetyConfig(BaseSettings):
    ...
    hhh_threshold: float = Field(
        ...
        description="HHH safety threshold"
    )
    max_violations: int = Field(
        default=3, ge=0, 
        description="Maximum safety violations allowed"
    )
    enable_filtering: bool = Field(
        default=True, 
        description="Enable HHH safety filtering"
    )
...
def get_execution_config() -> ExecutionConfig:
    """Backward-compat alias; prefer get_settings().execution."""
    return get_settings().execution 