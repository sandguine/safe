...
class IntegratedPipeline:
    ...
    async def run_cycle(self) -> PipelineMetrics:
        ...
        if self.enable_hhh_filter and self.hhh_filter:
            hhh_result = self.hhh_filter.evaluate_content(puzzle.content, "puzzle")
        ...
        solutions = [
            s for s in self.deduction_loop.solutions if s.puzzle_id == puzzle.id
        ]
        ...
        if self.enable_hhh_filter and self.hhh_filter:
            hhh_result = self.hhh_filter.evaluate_content(solution.content, "solution")
    ...
    def toggle_best_of_n(self):
        ...
        if self.best_of_n_sampler:
            ...
            print(
                f"Best-of-N sampling: {'ENABLED' if self.enable_best_of_n else 'DISABLED'}"
            )
        else:
    ...
    def get_pipeline_summary(self) -> Dict[str, Any]:
        ...
        overall_approval_rate = (
            approved_results / len(self.cycle_metrics) if self.cycle_metrics else 0
        )
        ...
        if self.best_of_n_sampler:
            best_of_n_metrics = [
                m.best_of_n_metrics for m in self.cycle_metrics if m.best_of_n_metrics
            ]
            if best_of_n_metrics:
                avg_reward = sum(
                    m.get("avg_reward", 0) for m in best_of_n_metrics
                ) / len(best_of_n_metrics)
                max_reward = max(m.get("max_reward", 0) for m in best_of_n_metrics)
                correct_rate = sum(
                    m.get("correct_rate", 0) for m in best_of_n_metrics
                ) / len(best_of_n_metrics)
        ...
        if self.hhh_filter:
            ...
            if hhh_metrics:
                ...
                approved_content = sum(
                    1 for m in hhh_metrics if m.get("safety_level") == "safe"
                )
                blocked_content = sum(
                    1 for m in hhh_metrics if m.get("safety_level") == "blocked"
                )
                warning_content = sum(
                    1 for m in hhh_metrics if m.get("safety_level") == "warning"
                )

                avg_helpful = sum(
                    m.get("avg_helpful_score", 0) for m in hhh_metrics
                ) / len(hhh_metrics)
                avg_harmless = sum(
                    m.get("avg_harmless_score", 0) for m in hhh_metrics
                ) / len(hhh_metrics)
                avg_honest = sum(
                    m.get("avg_honest_score", 0) for m in hhh_metrics
                ) / len(hhh_metrics)
