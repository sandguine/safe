...
class TransparencySystem:
    ...
    def __init__(
        self, model_name: str = "claude-3-5-sonnet-20241022", max_samples: int = 5
    ):
    ...
    def capture_refusal_rationale(
        ...
        if not evaluation_result.is_approved:
            ...
            if len(self.transparency_samples) > self.max_samples:
                self.transparency_samples = self.transparency_samples[
                    -self.max_samples :
                ]

        return sample

    def generate_transparency_samples(
        self, test_content: List[Dict[str, str]]
    ) -> List[TransparencySample]:
        """Generate transparency samples for flagged outputs"""

        print("🔍 Generating transparency samples...")
        ...
        for i, item in enumerate(test_content, 1):
            ...
            if not sample.evaluation_result.is_approved:
                ...
                print("    ✅ Flagged - rationale captured")
            else:
                print("    ✅ Approved")
...
DETAILED_RATIONALE: [comprehensive explanation]"""
        ...
        try:
            ...
            response = ask(
                prompt, model=self.model_name, max_tokens=1024, temperature=0.1
            )
            return response
    ...
    def display_transparency_samples(self) -> str:
        ...
        for i, sample in enumerate(self.transparency_samples, 1):
            ...
            report.append(
                f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(sample.timestamp))}"
            )
    ...
    def save_transparency_data(
        self, filepath: str = "results/transparency_samples.json"
    ):
    ...
    def generate_transparency_report(self) -> str:
        ...
        total_samples = len(self.transparency_samples)
        avg_helpful = (
            sum(s.evaluation_result.helpful_score for s in self.transparency_samples)
            / total_samples
        )
        avg_harmless = (
            sum(s.evaluation_result.harmless_score for s in self.transparency_samples)
            / total_samples
        )
        avg_honest = (
            sum(s.evaluation_result.honest_score for s in self.transparency_samples)
            / total_samples
        )
        avg_overall = (
            sum(s.evaluation_result.overall_score for s in self.transparency_samples)
            / total_samples
        )
        ...
        for reason, count in sorted(
            refusal_reasons.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        worst_sample = min(
            self.transparency_samples, key=lambda s: s.evaluation_result.overall_score
        )
        report.append(
            f"• Lowest overall score: {worst_sample.evaluation_result.overall_score:.3f}"
        )
        ...
        borderline_samples = [
            s
            for s in self.transparency_samples
            if 0.4 <= s.evaluation_result.overall_score <= 0.6
        ]
        ...
        avg_processing_time = (
            sum(s.processing_time for s in self.transparency_samples) / total_samples
        )
    ...
    def get_sample_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_scores": {
                "helpful": sum(
                    s.evaluation_result.helpful_score for s in self.transparency_samples
                )
                / len(self.transparency_samples),
                "harmless": sum(
                    s.evaluation_result.harmless_score
                    for s in self.transparency_samples
                )
                / len(self.transparency_samples),
                "honest": sum(
                    s.evaluation_result.honest_score for s in self.transparency_samples
                )
                / len(self.transparency_samples),
                "overall": sum(
                    s.evaluation_result.overall_score for s in self.transparency_samples
                )
                / len(self.transparency_samples),
            },
            "avg_processing_time": sum(
                s.processing_time for s in self.transparency_samples
            )
            / len(self.transparency_samples),
            "content_types": list(
                set(s.content_type for s in self.transparency_samples)
            ),
        }
