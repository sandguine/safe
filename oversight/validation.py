...
class StandardValidator:
    ...
    def log(self, message: str, level: str = "INFO", color: str = Colors.BLUE):
        ...
        print(
            f"{color_code}[{level}]{Colors.NC} [{timestamp}] {self.script_name}: {message}"
        )
        ...
        try:
            ...
            if self.working_dir.name == "oversight_curriculum":
                ...
                if oversight_dir.exists():
                    ...
                    self.log(
                        f"Changed to oversight_curriculum directory: {self.working_dir}",
                        "SUCCESS",
                    )
                    self.validation_results["working_directory"] = True
                    return True
        ...
        try:
            ...
            if api_key and api_key != "your-api-key-here":
                try:
                    # Simple test call
                    response = ask(
                        "Say 'hello'", model="claude-3-5-sonnet-20241022", max_tokens=10
                    )
