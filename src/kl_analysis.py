...
class KLAnalyzer:
    ...
    def _compute_kl_for_n(self, n: int, baseline_result: Dict[str, Any]) -> KLResult:
        ...
        accuracy = total_correct / total_puzzles
        accuracy_improvement = (accuracy - baseline_result['accuracy']) * 100  # percentage points
    ...
    def _compute_kl_divergence(self, 
                              ...
                              sample_distributions: List[Dict]) -> float:
        """
    ...
    def generate_report(self) -> str:
        ...
        for result in self.kl_results:
            report.append(f"{result.n_samples}\t{result.avg_reward:.3f}\t{result.kl_divergence:.4f}\t{result.accuracy_improvement:+.1f}")
        ...
        report.append(f"• Best accuracy improvement: {best_improvement.accuracy_improvement:+.1f}pp (n={best_improvement.n_samples})")
        
        # Find lowest KL divergence
        lowest_kl = min(self.kl_results, key=lambda r: r.kl_divergence)
        report.append(f"• Lowest KL divergence: {lowest_kl.kl_divergence:.4f} (n={lowest_kl.n_samples})")
        
        # Find best reward
        best_reward = max(self.kl_results, key=lambda r: r.avg_reward)
        report.append(f"• Best average reward: {best_reward.avg_reward:.3f} (n={best_reward.n_samples})")
