...
"""
    ...
    def run_main_application(self):
        ...
        try:
            ...
            self.log(f"  - Referee: ON (with oversight)", "INFO")
            self.log(f"  - Config puzzles: ON", "INFO")
            
            # Generate output filename with timestamp
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            output_file = self.working_dir / "results" / f"safe_run_{timestamp}.csv"
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Safe execution script for oversight curriculum")
