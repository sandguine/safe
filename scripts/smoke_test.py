...
sys.path.insert(0, str(Path(__file__).parent.parent / "oversight"))
...
try:
    ...
    print(f"Warning: Could not import oversight modules: {e}")
...
class SmokeTestSuite:
    ...
    def test_environment_setup(self) -> Dict[str, Any]:
        ...
        required_files = [
            "oversight/deduction_loop.py",
            "oversight/hhh_filter.py",
            "oversight/metrics.py",
            "oversight/referee.py",
            "requirements.txt",
        ]
    ...
    def test_imports(self) -> Dict[str, Any]:
        ...
        if not details["all_imports_successful"]:
            failed = [
                m for m, success in import_results.items() if not success
            ]
    ...
    def test_metrics_collector(self) -> Dict[str, Any]:
        ...
        if summary.approval_rate != 0.5:
            ...
            raise ValueError(
                f"Expected {expected} approval rate, got {actual}"
            )
